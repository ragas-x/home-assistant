...
import { acknowledgeReminder, addFamilyNote, addReminder, addShopping, getDashboard, initializeStore, setFamilyNotePinned, setMeal, snoozeReminder, startKitchenTimer, toggleRecord, updateFamilyNote, updateReminder, updateShopping } from '@/lib/store';
...
export async function POST(request: Request) {
  ...
  } else if (body.op === 'updateShopping' && typeof body.id === 'string' && typeof body.name === 'string' && body.name.trim()) {
    await updateShopping(body.id, body.name);
  ...
  } else if (body.op === 'updateReminder' && typeof body.id === 'string' && typeof body.title === 'string' && body.title.trim()) {
    await updateReminder(body.id, body.title, typeof body.dueAt === 'string' ? body.dueAt : undefined);
  ...
  } else if (body.op === 'updateFamilyNote' && typeof body.id === 'string' && typeof body.message === 'string' && body.message.trim()) {
    await updateFamilyNote(body.id, body.message);
